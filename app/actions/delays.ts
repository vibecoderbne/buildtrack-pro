...
function computeCascade(
  tasks: { id: string; current_start: string | null; current_end: string | null }[],
  ...
  while (queue.length > 0) {
    ...
    if (!pred.current_end) continue

    const newPredEnd = addDays(pred.current_end, predShift)

    for (const { taskId: succId, lagDays } of succMap.get(predId) ?? []) {
      const succ = taskMap.get(succId)
      if (!succ?.current_start) continue

      const requiredStart = addDays(newPredEnd, lagDays)
      const neededShift = Math.max(0, diffDays(requiredStart, succ.current_start))
...
export async function previewDelayCascade(
  ...
  const { data: rawTasks } = await supabase
    .from('tasks')
    .select('id, name, phase_id, current_start, current_end')
    .eq('project_id', projectId)
  ...
  for (const [taskId, shiftDays] of shifts) {
    ...
    if (!task?.current_start || !task.current_end) continue
    ...
    items.push({
      ...
      newStart: addDays(task.current_start, shiftDays),
      newEnd: addDays(task.current_end, shiftDays),
      shiftDays,
...
export async function saveDelayAndApplyCascade(
  ...
  if (shifts.size > 0) {
    await Promise.all(
      [...shifts.entries()].map(([taskId, shiftDays]) => {
        const task = tasks.find((t) => t.id === taskId)
        if (!task?.current_start || !task.current_end) return Promise.resolve()
        return supabase.from('tasks').update({
          current_start: addDays(task.current_start, shiftDays),
          current_end:   addDays(task.current_end, shiftDays),
        }).eq('id', taskId)
      })
    )
